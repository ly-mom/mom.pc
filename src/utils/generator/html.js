...
export function dialogWrapper(str) {
  return `<el-dialog v-bind="$attrs" v-on="$listeners" @open="onOpen" @close="onClose" title="Dialog Titile">
    ${str}
    <div slot="footer">
      <el-button @click="close">{{$t('usual_cancel')}}</el-button>
      <el-button type="primary" @click="handelConfirm">{{$t('MMS_Sure')}}</el-button>
    </div>
  </el-dialog>`
}
...
function buildFromBtns(conf, type) {
  ...
  if (conf.formBtns && type === 'file') {
    str = `<el-form-item size="large">
          ...
          <el-button @click="resetForm">{{$t('usual_reset')}}</el-button>
        </el-form-item>`
    if (someSpanIsNot24) {
      str = `<el-col :span="24">
          ${str}
