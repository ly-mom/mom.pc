<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item :label="$t('sc_jobName')" prop="jobName">
      ...
      <el-form-item :label="$t('sc_jobGroup')" prop="jobGroup">
      ...
      <el-form-item :label="$t('usual_status')" prop="status">
        <el-select v-model="queryParams.status" :placeholder="$t('usual_choose')" clearable size="small" style="width: 240px">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['monitor:job:remove']">{{ $t('usual_delete') }}</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" @click="handleClean" v-hasPermi="['monitor:job:remove']">清空</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" :loading="exportLoading" @click="handleExport" v-hasPermi="['monitor:job:export']">{{ $t('usual_export') }}</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-close" size="mini" @click="handleClose">{{ $t('usual_close') }}</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="jobLogList" @selection-change="handleSelectionChange">
      ...
      <el-table-column :label="$t('sc_jobName')" align="center" prop="jobName" :show-overflow-tooltip="true" />
      <el-table-column :label="$t('sc_jobGroup')" align="center" prop="jobGroup" :formatter="jobGroupFormat" :show-overflow-tooltip="true" />
      <el-table-column :label="$t('sc_invokeTarget')" align="center" prop="invokeTarget" :show-overflow-tooltip="true" />
      <el-table-column label="日志信息" align="center" prop="jobMessage" :show-overflow-tooltip="true" />
      <el-table-column :label="$t('usual_status')" align="center" prop="status" :formatter="statusFormat" />
    ...
    <!-- 调度日志详细 -->
    <el-dialog :title="$t('sc_title2')" :visible="open" width="700px" append-to-body>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="open = false">{{ $t('usual_close') }}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除调度日志编号为"' + jobLogIds + '"的数据项?', this.$t('usual_warning'), {
    ...
    /** 清空按钮操作 */
    handleClean() {
      this.$confirm('是否确认清空所有调度日志数据项?', this.$t('usual_warning'), {
        confirmButtonText: this.$t('usual_confirm'),
        cancelButtonText: this.$t('usual_cancel'),
        type: 'warning',
      })
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有调度日志数据项?', this.$t('usual_warning'), {
        confirmButtonText: this.$t('usual_confirm'),
        cancelButtonText: this.$t('usual_cancel'),
        type: 'warning',
      })
